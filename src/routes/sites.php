...
Route::group(['middleware' => ['web', 'role:1,3,4,5,8']], function () {
    ...
    Route::get('site/updated-status', function (Request $request) {
        return view('site::site_status_update');  // Or any view you want to return on GET
    })->name('site.status.update');
    
    // Handling POST request
    Route::post('site/updated-status', function (Request $request) {
        // When request is from Site_status_update.blade
        if (isset($request->batch_status_update)) {
            ...
            // Create the HTML table for status updates
            ...
            foreach ($request->batch_status_update as $key => $v) {
                if (array_key_exists('site_id', $v)) {
                    ...
                    $data->save();
                    Tritiyo\Site\Models\Site::where('id', $v['site_id'])->update(['completion_status' => $request->input('status')]);
                    ...
                    $html .= '<td>' . $request->input('status') . '</td>';
                    $html .= '</tr>';
